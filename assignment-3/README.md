...
## Program:
    1. secondary_structure_ssplitter.py
        - use argsparse terminal inputs for program
        - infile required
        - give the output of 2 files from the one provided input
        - separate protein sequence data from secondary structure data
    2. nt_fasta_stats.py
        - use argsparse terminal inputs for program
        - infile required and outfile required
        - give output file of descriptive counts for influenza sequence

## Text Files (numbers correspond to program number):
    - input files provided with fasta format
        1. ss.txt
        2. influenza.fasta
    - output files
        1. hardcoded into program
        2. specified in terminal by argparse argument
